        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="m-4 fixed top-4 right-4 z-50 borde-2 hover:border-primary
              ...
              {navLinks.map(link => {
                ...
                return (
                  <a
                    ...
                    className={cn(
                      'text-neutral-300 flex items-center gap-2 hover:text-primary',
                      'transition-colors duration-200 text-base',
                      active === link.link && 'text-primary'
                    )}
                  >
                    <Icon className="size-6" /> {link.label}
                  </a>
                );
              })}
            </nav>
